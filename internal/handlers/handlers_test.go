...
func TestHandlers(t *testing.T) {
	...
	for _, e := range theTests {
		...
		if resp.StatusCode != e.expectedStatusCode {
			t.Errorf("expected status code %d, got %d", e.expectedStatusCode, resp.StatusCode)
		}
...
func TestRepository_Reservation(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Errorf("Reservation handler returned the wrong response code: got %d, wanted %d", rr.Code, http.StatusOK)
	}
